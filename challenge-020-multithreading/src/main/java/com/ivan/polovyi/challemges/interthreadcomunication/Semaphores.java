package com.ivan.polovyi.challemges.interthreadcomunication;
...
/*
Semaphore:
  - used to restrict number of "users" to a particular resource
    or group of resources
  - unlike the locks that only restrict one "user" per resource
    semaphore can restrict any given number of users to a resource

    -- wait() - causes current thread to wait until another thread wakes up
      - in this state, thread in not consuming any CPU
    ...
    -- To call wait(), notify(), notifyAll() - we need to acquire the monitor
       of that object
